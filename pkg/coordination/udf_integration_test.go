...
func TestUDFIntegration_FullWorkflow(t *testing.T) {
	...
	t.Run("GetUDFVersions", func(t *testing.T) {
		...
		// Check first version object contains version field
		firstVersion := versions[0].(map[string]interface{})
		assert.Equal(t, "1.0.0", firstVersion["version"])
	})
	...
	t.Run("DeleteUDF", func(t *testing.T) {
		...
		err := json.NewDecoder(w.Body).Decode(&response)
		require.NoError(t, err)
		assert.Equal(t, "UDF deleted successfully", response["message"])
		assert.Equal(t, "test_filter", response["name"])
		assert.Equal(t, "1.0.0", response["version"])
	})
...
func createMinimalWASMModule() string {
	// Minimal valid WASM module with a dummy exported function
	// This module exports a function "udf_main" that returns i32(1)
	//
	// WAT (WebAssembly Text) equivalent:
	// (module
	//   (func $udf_main (export "udf_main") (result i32)
	//     i32.const 1
	//   )
	// )
	//
