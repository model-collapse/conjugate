...
#############################################################################
# CONJUGATE Kubernetes Deployment Script
#
# A comprehensive deployment script for CONJUGATE search engine that supports:
...
#   ./deploy-k8s.sh --mode raft --profile prod --namespace conjugate-prod
#
...
# Default values
MODE="${MODE:-auto}"
NAMESPACE="${NAMESPACE:-conjugate}"
PROFILE="${PROFILE:-dev}"
CLUSTER_NAME="${CLUSTER_NAME:-conjugate}"
WAIT="${WAIT:-true}"
...
usage() {
    cat << EOF
${BOLD}CONJUGATE Kubernetes Deployment Script${NC}
...
${BOLD}EXAMPLES:${NC}
    ...
    $0 --mode raft --profile prod --namespace conjugate-prod
...
metadata:
  name: $NAMESPACE
  labels:
    app.kubernetes.io/name: conjugate
...
metadata:
  name: ${CLUSTER_NAME}-operator
rules:
- apiGroups: ["conjugate.io"]
  resources: ["conjugateindices", "conjugateclusters"]
  verbs: ["get", "list", "watch", "create", "update", "patch", "delete"]
- apiGroups: ["conjugate.io"]
  resources: ["conjugateindices/status", "conjugateclusters/status"]
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: operator
        image: conjugate/operator:latest
        imagePullPolicy: IfNotPresent
        env:
        - name: CONTROL_PLANE_MODE
          value: "k8s"
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: coordination
        image: conjugate/coordination:latest
        imagePullPolicy: IfNotPresent
        env:
        - name: CONTROL_PLANE_MODE
          value: "$MODE"
...
uninstall() {
    section "Uninstalling CONJUGATE"

    warn "This will delete all CONJUGATE resources in namespace: $NAMESPACE"
...
EOF
    ...
    if [[ "$DRY_RUN" != "true" ]]; then
        ...
        cat << EOF
${GREEN}✓${NC} CONJUGATE cluster deployed successfully!
