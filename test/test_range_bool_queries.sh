...
# Test configuration
TEST_DIR="/tmp/conjugate-range-bool-test"
...
echo ""
echo "2. Starting CONJUGATE cluster..."
echo "   Master GRPC port: $MASTER_GRPC_PORT"
echo "   Data GRPC port: $DATA_GRPC_PORT"
echo "   Coordination HTTP port: $COORDINATION_PORT"

$PROJECT_ROOT/bin/conjugate-master --config "$TEST_DIR/config/master.yaml" > "$TEST_DIR/logs/master.log" 2>&1 &
MASTER_PID=$!
echo -e "${GREEN}   ✓ Master node started (PID: $MASTER_PID)${NC}"

sleep 2

$PROJECT_ROOT/bin/conjugate-data-new --config "$TEST_DIR/config/data.yaml" > "$TEST_DIR/logs/data.log" 2>&1 &
DATA_PID=$!
echo -e "${GREEN}   ✓ Data node started (PID: $DATA_PID)${NC}"

sleep 2

$PROJECT_ROOT/bin/conjugate-coordination --config "$TEST_DIR/config/coordination.yaml" > "$TEST_DIR/logs/coordination.log" 2>&1 &
COORD_PID=$!
