...
cleanup() {
    ...
    if [ "$TEST_FAILED" = "true" ]; then
        echo "Test failed - saving logs to /tmp/conjugate-docret-test-FAILED"
        cp -r /tmp/conjugate-docret-test /tmp/conjugate-docret-test-FAILED 2>/dev/null || true
    fi
    ...
    # Clean up test data
    rm -rf /tmp/conjugate-docret-test
...
# Test configuration
TEST_DIR="/tmp/conjugate-docret-test"
...
"$PROJECT_ROOT/bin/conjugate-master" \
...
"$PROJECT_ROOT/bin/conjugate-data-new" \
...
"$PROJECT_ROOT/bin/conjugate-coordination" \
