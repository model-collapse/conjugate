#!/bin/bash
#
# Failure Testing for CONJUGATE Cluster (Realistic Version)
...
DATA_DIR="/tmp/conjugate-failure-test-v2"
...
echo "=========================================="
echo " CONJUGATE Failure Testing Suite v2"
echo "=========================================="
echo ""
...
cleanup() {
    echo ""
    echo "Cleaning up..."
    pkill -f "conjugate-master" || true
    pkill -f "conjugate-coordination" || true
    pkill -f "conjugate-data" || true
    sleep 2
    rm -rf "$DATA_DIR"
...
$BIN_DIR/conjugate-master --config "$DATA_DIR/config/master.yaml" \
...
$BIN_DIR/conjugate-data --config "$DATA_DIR/config/data.yaml" \
...
$BIN_DIR/conjugate-coordination --config "$DATA_DIR/config/coordination.yaml" \
...
if [ $TESTS_FAILED -eq 0 ]; then
    ...
    echo "=========================================="
    echo ""
    echo "The CONJUGATE cluster demonstrates excellent resilience:"
