...
DATA_DIR="/tmp/conjugate-test-rb"
...
$PROJECT_ROOT/bin/conjugate-master --config "$DATA_DIR/config/master.yaml" \
    > "$LOG_DIR/master.log" 2>&1 &
MASTER_PID=$!
echo "   Master started (PID: $MASTER_PID)"
sleep 3

$PROJECT_ROOT/bin/conjugate-data-new --config "$DATA_DIR/config/data.yaml" \
    > "$LOG_DIR/data.log" 2>&1 &
DATA_PID=$!
echo "   Data node started (PID: $DATA_PID)"
sleep 3

$PROJECT_ROOT/bin/conjugate-coordination --config "$DATA_DIR/config/coordination.yaml" \
